...
func TestRunWithRetry(t *testing.T) {
	...
	tests := []struct {
		...
		{
			name: "BEB strategy and function fails always",
			options: RetryOptions{
				RetryCount:    3,
				WaitSeconds:   1,
				RetryStrategy: "beb",
			...
			funcToRun:      RunFunc,
			args:           []interface{}{5},
			expectedError:  errors.New("Failure after retries"),
			expectedResult: nil,
		},
		{
			name: "Non-BEB strategy and function succeeds after few runs",
			options: RetryOptions{
				RetryCount:    5,
				WaitSeconds:   1,
				RetryStrategy: "non-beb",
			},
			funcToRun:      RunFunc,
			args:           []interface{}{5},
			expectedError:  nil,
		...
		{
			name: "Non-BEB strategy and function fails always",
			options: RetryOptions{
				RetryCount:    3,
				WaitSeconds:   1,
				RetryStrategy: "non-beb",
